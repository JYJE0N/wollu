...
export const PracticeToggle = memo<PracticeToggleProps>(
  ({ practiceMode, onToggle, currentLanguage, disabled = false }) => {
    ...
    const knobSize = 28; // 28 → 32로 증가
    const knobOffset = 4; // 4 → 2로 줄여서 노브가 더 크게 보이게
    ...
    return (
      ...
      >
        {/* 왼쪽 레이블 (문장) */}
        <div
          className="absolute left-0 w-1/2 h-full flex items-center justify-center text-[10px] font-bold text-white pointer-events-none select-none transition-opacity duration-200"
        ...
        {/* 오른쪽 레이블 (단어) */}
        <div
          className="absolute right-0 w-1/2 h-full flex items-center justify-center text-[10px] font-bold text-white pointer-events-none select-none transition-opacity duration-200"
          style={{ opacity: !isSentence ? 1 : 0.5 }}
        >
        ...
        >
          {isSentence ? <BookOpen className="w-4 h-4" /> : <Zap className="w-4 h-4" />}
        </motion.div>
      </button>
    );
  }
